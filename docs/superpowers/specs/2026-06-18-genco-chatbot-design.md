...
| 2 | RAG layer | P1 | `schema.sql` (vector ext, `kb_documents`, `match_documents`, HNSW index — as-built correction: this row originally specified ivfflat, but the implementation deliberately uses HNSW because the ~20-40-chunk KB leaves most ivfflat lists empty, degrading single-probe recall; HNSW needs no list tuning or post-insert training. Rationale recorded in `backend/app/rag/schema.sql` and VERIFICATION.md check 1 — do not "fix" it back to ivfflat), `embeddings.py`, `ingest.py` (CLI), `retrieve.py`, seeded `knowledge_base/*.md`, `test_retrieval.py` |
