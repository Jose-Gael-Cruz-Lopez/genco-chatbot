...
- [x] **Step 3: Write the visitor polling endpoint**
...
- [x] **Step 4: Add the `live` flag to `/chat`**
...
- [x] **Step 5: Update the contract tests**
...
- [x] **Step 1: Style the agent bubble**
...
- [x] **Step 2: Render agent turns distinctly**
...
- [x] **Step 4: Start and stop it from the chat response**
...
- [x] **Step 5: Teach the stub about live mode**
...
- [x] **Step 1: Implement the pane**
- [x] **Step 2: Verify by hand against a running server**
- [x] **Step 3: Commit**
...
- [x] **Step 1: README** — how live chat works, that it is offered only while the team is toggled available, the three timing settings, and that turning the toggle off (or closing the tab) returns the bot to the email-only behaviour.
- [x] **Step 2: VERIFICATION** — a full live chat end to end, then each of the four end reasons, each confirming a lead arrives with the transcript.
- [x] **Step 3: LAUNCH_CHECKLIST** — re-apply `schema.sql` for `agent_presence` and `live_chats`; confirm that with the toggle off a visitor gets the email flow unchanged.
...
- [x] `cd backend && python -m pytest tests/ -q` — all pass, 202 baseline not regressed.
- [x] `grep -rnE "app\.llm|rag\.embeddings|embed_text|embed_batch" backend/app/live/` returns nothing.
- [x] `git diff --stat backend/requirements.txt` is empty.
- [x] `/chat` returns all five keys on every path, always HTTP 200.
- [x] With the team unavailable, a visitor tapping "No — ask the team" gets today's email flow, byte for byte.
- [x] Each of the four end reasons ends the chat and notifies its lead exactly once.
- [x] `grep -nE "innerHTML\s*=" portal/dist/portal.html widget/dist/widget.js` returns nothing.
